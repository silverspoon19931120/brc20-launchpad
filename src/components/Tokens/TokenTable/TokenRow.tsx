...
const StyledTokenRow = styled.div<{
  ...
  display: grid;
  font-size: 16px;
  grid-template-columns: 1fr 5fr 3fr 3fr 3fr 3fr 4fr 4fr;
  line-height: 24px;
...
export const HEADER_DESCRIPTIONS: Record<TokenSortMethod, ReactNode | undefined> = {
  ...
  [TokenSortMethod.MKCAP]: undefined,
  ...
  [TokenSortMethod.SUPPLY]: undefined,
  [TokenSortMethod.HOLDERS]: undefined,
}
...
export function TokenRow({
  ...
  const rowCells = (
    <>
      ...
      <TvlCell sortable={header}>{mkcap}</TvlCell>
      <VolumeCell sortable={header}>{volume}</VolumeCell>
      <VolumeCell sortable={header}>{supply}</VolumeCell>
      <VolumeCell sortable={header}>{holder}</VolumeCell>
...
export function HeaderRow() {
  return (
    <TokenRow
      ...
      mkcap={<HeaderCell category={TokenSortMethod.MKCAP} />}
      volume={<HeaderCell category={TokenSortMethod.VOLUME} />}
      supply={<HeaderCell category={TokenSortMethod.SUPPLY} />}
      holder={<HeaderCell category={TokenSortMethod.HOLDERS} />}
    // sparkLine={null}
    />
  )
}
...
export function LoadingRow(props: { first?: boolean; last?: boolean }) {
  return (
    <TokenRow
      ...
      supply={<LoadingBubble />}
      holder={<LoadingBubble />}
      {...props}
    />
  )
}
    ...
    <>
      <div ref={ref} className='table'>
        ...
        >
          <TokenRow
            ...
            mkcap={<ClickableContent>{formatDollar({ num: token.market?.totalValueLocked?.value })}</ClickableContent>}
            volume={<ClickableContent>{formatDollar({ num: token.market?.volume?.value })}</ClickableContent>}
            supply={<ClickableContent>{formatDollar({ num: token.market?.volume?.value })}</ClickableContent>}
            holder={<ClickableContent>{formatDollar({ num: token.market?.volume?.value })}</ClickableContent>}
            // sparkLine={
            //   <SparkLine>
            //     <ParentSize>
            //       {({ width, height }) =>
      ...
      <div className="box">
        <div className="header">
          ...
          <p>{formatDollar({ num: token.market?.volume?.value })} Holders</p>
        ...
        <div className="body">
          ...
          <div className="first">
            <label>Price</label>
            <p className='p'>{formatDollar({ num: token.market?.price?.value, isPrice: true, lessPreciseStablecoinValues: true })}</p>
            <p className='priceColor'> {formattedDelta}</p>
          </div>

          <div className="first">
            <label>Market Cap</label>
            <p className='p'>$520,648,120.67</p>

          </div>

          <div className="first">
            <label>Volume(24h)</label>
            <p className='p'>{formatDollar({ num: token.market?.volume?.value })}</p>

          </div>

          <div className="first supply">
            <label>Supply</label>

            <div className="relativeDiv">
              <p>{formatDollar({ num: token.market?.volume?.value })}<span>(100%)</span></p>
